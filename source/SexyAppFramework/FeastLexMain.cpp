...
public:
    ...
    CLexBitSet() { if (defaultSize) Init(defaultSize); else Init(2048); } // hc | 62 (Match)
    CLexBitSet(int s) { Init(s); } //63 (Match)
    ~CLexBitSet() { if (data) FREE(data); } //64 (Match)
	...
	inline CLexBitSet& operator += (const dword v) // add element | 74-80
	{
	...
	inline CLexBitSet& operator -= (const dword v) // remove element | 82-88
	{
#ifdef BITSET_RANGECHECK
		if (v < size)
#endif
	...
	inline CLexBitSet& operator = (const CLexBitSet& other) // assignment | 90-96
	{
	...
	inline CLexBitSet& operator |= (const CLexBitSet& other) // union | 98-105
	{
		dword lowest = sizeInBytes;
		if (other.sizeInBytes < lowest)
			lowest = other.sizeInBytes;
	...
	inline int operator == (const CLexBitSet& other) // equivalent | 135-143
	{
		dword lowest = sizeInBytes;
		if (other.sizeInBytes < lowest)
			lowest = other.sizeInBytes;
	...
	inline void Invert() //169-172
	{
		for (dword i=0; i<sizeInBytes; i++)
			data[i] ^= 0xFF;
	}
	inline void Empty() //174-178
	{
		memset(data, 0, sizeInBytes);
        //for (dword i=0; i<sizeInBytes; i++)
		//	data[i] = 0;
	...
	inline int Contains(dword v) //186-192
	{
#ifdef BITSET_RANGECHECK
		if (v < size)
#endif
...
private:
    ...
    void operator delete(void* ptr) { } //228
public:
	void* operator new(size_t size, void* ptr) { return(ptr); }
	void operator delete(void*, void*) { }
    void* operator new(size_t size, CLexNfaNodeFactory& factory) { return(factory.AllocNode()); } //232
    void operator delete(void* ptr, CLexNfaNodeFactory& factory) { FEAST_ERROR("CLexNfaNode: illegal operator delete"); } //233
...
CLexNfaNodeFactory::CLexNfaNodeFactory(dword nodeCount) //254-259
{
	numNodes = 0;
	maxNodes = nodeCount;
...
CLexNfaNodeFactory::~CLexNfaNodeFactory() //261-268
{
	if (nodes)
	{
...
CLexNfaNode* CLexNfaNodeFactory::AllocNode() //270-275
{
...
CLexNfaNode* CLexNfaNodeFactory::NodeForId(dword id) { return(&nodes[id]); } //276
dword CLexNfaNodeFactory::IdForNode(CLexNfaNode* node) { return((dword)(node - nodes)); } //277
...
public:
	void* operator new(size_t size, void* ptr) { return(ptr); }
	void operator delete(void*, void*) { }
    void* operator new(size_t size, CLexNfaExprFactory& factory) { return(factory.AllocExpr()); } //305
    void operator delete(void* ptr, CLexNfaExprFactory& factory) { FEAST_ERROR("CLexNfaExpr: illegal operator delete"); } //306

    CLexNfaNode* first;
    CLexNfaNode* last;

    CLexNfaExpr() { first = last = NULL; } //311
};

CLexNfaExprFactory::CLexNfaExprFactory(dword exprCount) { numExprs = 0; maxExprs = exprCount; exprs = ALLOC(CLexNfaExpr, maxExprs); /*for (dword i=0;i<maxExprs;i++) new(&exprs[i]) CLexNfaExpr;*/ } //314
CLexNfaExprFactory::~CLexNfaExprFactory() { if (exprs) FREE(exprs); } //315-322
CLexNfaExpr* CLexNfaExprFactory::AllocExpr() //317-322
{
...
CLexNfaExpr* CLexNfaExprFactory::ExprForId(dword id) { return(&exprs[id]); } //323
dword CLexNfaExprFactory::IdForExpr(CLexNfaExpr* expr) { return((dword)(expr - exprs)); } //324
...
public:
    ...
    inline CLexDfaAcceptItem& operator = (CLexDfaAcceptItem& i) //338-341
    {
        tag = i.tag; flags = i.flags; tokenFunc = i.tokenFunc; priority = i.priority;
        return(*this);
    }
    inline int operator == (const CLexDfaAcceptItem& i) //343-345
    {
...
public:
    ...
    bool Owns(sdword state, sdword* index) //399-410
    {
	...
	bool Remove(sdword state) //432-440
	{
		sdword index;
		if (!Owns(state, &index))
			return(0);
	...
	sdword Next(sdword start, sdword *nextstart=NULL) //442-451
	{
		sdword i;
...
public:
    ...
    inline CLexDfaPartition& operator[] (int i) //478-480
    {
        return(partitions[i]);
    }
    ...
    void SetDFA(CLexDfaTable* dfa) //493-497
    {
        useDFA = dfa;
        for (sdword i=0;i<DFA_MAXSTATES;i++)
            partitions[i].useDFA = dfa;
    }
    void MoveTo(sdword state, sdword part) //499-503
    {
        if (useDFA->stateParts[state] != -1)
            partitions[useDFA->stateParts[state]].Remove(state);
        partitions[part].Add(state);
    }
    void MoveToCurrent(sdword state) //505-510
    {
        if (partAdd)
            curPartition++;
        partAdd = 0;
...
public:
    ...
    void SetIntercept(dword inTag, void (*inIntercept)(ILexLexer*, SLexToken*)) //563-582
    {
        if (!inTag)
            return;
        if (!finalized)
...
// NFA Expressions -----------------------------------------
    ...
    CLexNfaExpr* ExprCreate(byte inEdge) //629-645
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
        CLexNfaExpr* e;
    ...
    CLexNfaExpr* ExprCreate(CLexBitSet& inEdgeSet) //647-663
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
        CLexNfaExpr* e;
    ...
    void ExprOr(CLexNfaExpr* dest, CLexNfaExpr* src) //665-681
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
    ...
    void ExprCat(CLexNfaExpr* dest, CLexNfaExpr* src) //683-689
    {
	    dest->last->edge = 0;
    ...
    void ExprOneOrMore(CLexNfaExpr* dest) //691-704
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
    ...
    void ExprZeroOrMore(CLexNfaExpr* dest) //706-720
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
    ...
    void ExprZeroOrOne(CLexNfaExpr* dest) //722-736
    {
        CLexNfaNode* n1;
        CLexNfaNode* n2;
    ...
    void RegisterExpr(CLexNfaExpr* e, dword tag) //753-771
    {
        CLexNfaNode* n;
...
// Regex Expressions -----------------------------------------
    ...
    byte RegexLetter(char*& regex, bool* outLiteral) //777-801
    {
        byte letter;
    ...
    bool RegexGroup(char*& regex, CLexBitSet& s) //803-847
    {
        byte a, b;
        bool literal;
    ...
    CLexNfaExpr* RegexRange(char*& regex) //849-863
    {
        bool negate = false;
        CLexBitSet s(256);
    ...
    CLexNfaExpr* RegexFactorPrime(char*& regex) //865-910
    {
        CLexNfaExpr* e;
        byte letter;
        bool literal;
    ...
    CLexNfaExpr* RegexFactor(char*& regex) //912-939
    {
    ...
    CLexNfaExpr* RegexTerm(char*& regex) //941-949
    {
    ...
    CLexNfaExpr* RegexExpr(char*& regex) //951-965
    {
    ...
    bool RegisterToken(char* inRegex, dword inTag, char* outError) //967-1002 (TODO)
    {
        if (!inRegex)
        {
            if (outError)
...
// Calculations -----------------------------------------
    ...
    inline void ComputeEpsilonClosure(CLexBitSet& inSet) //1008-1036
    {
    ...
    inline void ComputeMove(CLexBitSet& inSet, byte inEdge) //1039-1061
    {
        CLexNfaNode* n;
        dword i;
...
// Token Retrieval -----------------------------------------
    ...
    dword GetTokenNFA(bool advance, SLexToken* outToken) //1297-1417
    {
    ...
    dword GetToken(bool advance, SLexToken* outToken) //1420-1540
    {
	...
	// ILexLexer
	NBool LexDestroy() //1544-1547
	{
		delete this;
		return(1);
	}
	NBool LexCaseSensitivity(NBool inIsCaseSensitive) //1549-1552
	{
		isCaseSensitive = (inIsCaseSensitive!=0);
		return(1);
	}
	NBool LexRegisterToken(NDword inTag, const NChar* inRegex) //1554-1556
	{
	    return(RegisterToken((char*)inRegex, inTag, lexErrorStr));
	}
	NByte LexTokenPriority(NByte inPriority) //1558-1562
	{
		NByte oldPriority = activePriority;
		activePriority = inPriority;
		return(oldPriority);
	}
	NBool LexTokenIntercept(NDword inTag, void (*inIntercept)(ILexLexer*, SLexToken*)) //1564-1567
	{
		SetIntercept(inTag, inIntercept);
		return(1);
	}
	NBool LexFinalize() //1569-1572
	{
		FinalizeDFA();
		return(1);
	}
	NBool LexSetText(const NChar* inText, NDword inLine=0, NDword inColumn=0, NDword inTabColumns=8) //1574-1582
	{
		textPtr = (char*)inText;
		lineCount = inLine;
	...
	NChar* LexGetText(NDword* outLine=0, NDword* outColumn=0) //1584-1588
	{
		if (outLine) *outLine = lineCount;
		if (outColumn) *outColumn = columnCount;
		return(textPtr);
	}
	NChar LexGetChar(NBool inAdvance=true) //1590-1603
	{
		if (!textPtr)
			return(0);
		if (!inAdvance)
	...
	NDword LexGetToken(SLexToken* outToken, NBool inAdvance=true) //1605-1607
	{
	    return(GetToken(inAdvance, outToken));
	}
	char* LexGetLastError() //1609-1611
	{
		return(lexErrorStr);
	}
	const char* LexGetStockRegex(ELexStockRegex inStockRegex) //1613-1629
	{
		switch(inStockRegex)
		{
...
//    GLOBAL FUNCTIONS
//============================================================================
ILexLexer* ILexLexer::LexCreate() //1645-1647
{
