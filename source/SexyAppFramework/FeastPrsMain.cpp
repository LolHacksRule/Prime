...
public:
	...
	void* operator new (size_t size) { sNodeCount++; return(ALLOC(NByte, (NDword)size)); } //65
	void operator delete (void* ptr) { sNodeCount--; FREE(ptr); } //66
	...
	// IPrsNode
	NBool NodeDestroy() //95-98
	{
		delete this;
		return(1);
	}
	NDword NodeGetTag() { return(tag); } //99
	SLexToken* NodeGetToken() { return(token.mTag ? &token : NULL); } //100
	IPrsNode* NodeGetParent() { return(parent); } //101
	NDword NodeGetChildCount() { return(childCount); } //102
	IPrsNode* NodeGetChild(NDword inIndex) //104-108
	{
		if (inIndex >= childCount)
			return(NULL);
		return(children[inIndex]);
	}
	void NodeSetAttr(IPrsAttr* inAttr) { attr = inAttr; } //109
	IPrsAttr* NodeGetAttr() { return(attr); } //110
};
...
public:    
	...
	void* operator new (size_t size) { sNodeCount++; return(ALLOC(NByte, (NDword)size)); } //128
	void operator delete (void* ptr) { sNodeCount--; FREE(ptr); } //129
	...
	NBool NodeDestroy() //149-152
	{
		delete this;
		return(1);
	}
	const NChar* NodeGetName();
	CPrsASTNode* NodeGetASTNode() { return(astNode); } //154
	NBool NodeSetASTNode(CPrsASTNode* inAttr) { astNode = inAttr; return(1); } //155
	SLexToken* NodeGetToken() { return(token.mTag ? &token : NULL); } //156
	CPrsCSTNode* NodeGetParent() { return(parent); } //101
	CPrsCSTNode* NodeGetFirstChild() { return(firstChild); }
	CPrsCSTNode* NodeGetNext() { return(nextSibling); } //159
};
...
public:
    ...
    int operator == (CPrsParseProd& p) //194-206
    {
...
public:
    ...
    void SetName(char* name) //223-233
    {
        if (ruleName)
        {
            FREE(ruleName);
    ...
    void SetOrgName(char* name) //236-246
    {
        if (ruleNameOrg)
        {
            FREE(ruleNameOrg);
    ...
    void SetAlias(char* name) //249-259
    {
        if (ruleAlias)
        {
            FREE(ruleAlias);
...
const NChar* CPrsCSTNode::NodeGetName() //285-295
{
	if (!ruleInternal)
		return("");
...
public:
    ...
    inline int Matches(CPrsParseStateProd& p, dword dotAdd) // same as == but with unknown comparison dotPos | 312-314
    {
...
{
private:	
	static int ProdSortCompare(const void* arg1, const void* arg2) //321-329
	{
...
public:
	...
	~CPrsParseState() { if (nextRuleStates) FREE(nextRuleStates); } //338

	int Matches(CPrsParseState& s, int groupMatch, dword dotAdd) //341-357
	{
	...
	void SortStateProductions() //360-372
	{
...
public:
    ...
    CPrsParseRule* MakeRule(char* name, bool* existed, bool addable) //403-433
    {
	    CPrsParseRule* rule;
    ...
    CPrsParseRule* RuleForLexTag(dword tag) //436-443
    {
	    for (dword i=0;i<rulePoolIndex;i++)
	    {
    ...
    void RegisterTerminal(char* name, dword tag, dword astNodeTag, /*FPrsTermFunc*/void* event) //446-460
    {
        if (!name || !tag)
    ...
    void InitProductionLexer() //463-480
    {
        if (!prodLexer)
        {
    ...
    void RegisterNonTerminal(char* name, char* definition, dword astNodeTag, /*FPrsRuleFunc*/void* event) //483-546
    {
    ...
    void ResolveProductionItemRules() //549-638
    {
	    for (dword i=0;i<rulePoolIndex;i++)
	    {
    ...
    void RecursiveFireEvents(CPrsCSTNode* node) //806-900
    {
    ...
    void FinalizeParser(char* startRuleName="start") //903-917
    {
	    char buf[256];
	...
	void DestroyParseNode(CPrsCSTNode* node) //920-935
	{
    ...
    CPrsCSTNode* Parse(char* text, dword inTabSpacing, char* outError, dword* outErrorLine, dword* outErrorColumn, TDatArray<CPrsCSTNode*>& nodeLog) //938-1361
    {
	    SLexToken token;
	...
	// IPrsParser
	NBool PrsDestroy() //1395-1398
	{
		delete this;
		return 1;
	}
	ILexLexer* PrsGetLexer() //1400-1402
	{
		return(lexer);
	}
	bool PrsRegisterT(const char* inRule, unsigned long inLexTag, unsigned long inNodeTag) //1404-1407
	{
		RegisterTerminal((char*)inRule, inLexTag, inNodeTag, NULL);
		return(1);
	}
	bool PrsRegisterNT(const char* inRule, const char* inProduction, unsigned long inNodeTag) //1409-1412
	{
		RegisterNonTerminal((char*)inRule, (char*)inProduction, inNodeTag, NULL);
		return(1);
	}
	NBool PrsBuild() //1414-1419
	{
		if (finalized)
	...
	NBool PrsSetRuleAlias(const NChar* inRule, const NChar* inAlias) //1421-1430
	{
		if (!inRule)
			return(0);
		bool existed;
	...
	IPrsNode* PrsExecute(const NChar* inText, NDword inTabSpacing=8) //1432-1452
	{
	...
	char* PrsGetLastError(unsigned long* outLine=0, unsigned long* outColumn=0) //1455-1459
	{
...
void *CPrsParseState::operator new(size_t size, CPrsParser* parser, dword numRules) //1463-1473
{
...
void *CPrsParseStateProd::operator new(size_t size, CPrsParser* parser, dword numRules) //1476-1485
{
...
//    GLOBAL FUNCTIONS
//============================================================================
IPrsParser* IPrsParser::PrsCreate(unsigned long inLookAhead) //1500-1504
{
