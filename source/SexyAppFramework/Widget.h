...
class Widget : public WidgetContainer //Add gamepad and touch
{
public:			
	bool					mVisible;
	bool					mMouseVisible;		
	...
	bool					mIsGamepadSelection;
	Widget*					mGamepadParent;
	Widget*					mGamepadLinkUp;
	Widget*					mGamepadLinkDown;
	Widget*					mGamepadLinkLeft;
	Widget*					mGamepadLinkRight;
...
public:
	...
	virtual void			SetGamepadLinks(Widget* up, Widget* down, Widget* left, Widget* right);
	virtual void			SetGamepadParent(Widget* theParent);
	...
	virtual void			GamepadButtonDown(GamepadButton theButton, int thePlayer, GamepadExta theFlags);
	virtual void			GamepadButtonUp(GamepadButton theButton, int thePlayer, GamepadExta theFlags);
	virtual void			GamepadAxisMove(GamepadAxis theAxis, int thePlayer, float theAxisValue); //GamepadAxis to float in recent
#else
	...
	//////// Layout functions
	int						Left()							{ return mX; }  //131
	int						Top()							{ return mY; } //132
	int						Right()							{ return mX + mWidth; }
	int						Bottom()						{ return mY + mHeight; }
	int						Width()							{ return mWidth; } //135
	int						Height()						{ return mHeight; } //136
