...
bool Sexy::Quantize8Bit(const uint32* theSrcBits, int theWidth, int theHeight, uchar* theDestColorIndices, uint32* theDestColorTable) //This file matches 1:1 to the original (according to PL_D.dll) if you remove this comment.
{
	int aSize = theWidth*theHeight;

	int aColorTableSize = 0;
		
	uint32 aSearchTable[256];
	...
	{
		uint32 aColor = theSrcBits[anIdx];		
		...
		for (;;)
		{	
			uint32 aCheckColor = aSearchTable[aMiddlePos];
