...
std::string					SEHCatcher::mErrorTitle; //35
std::string					SEHCatcher::mErrorText; //36
std::string					SEHCatcher::mUserText; //37
std::string					SEHCatcher::mUploadFileName; //38
...
HTTPTransfer				SEHCatcher::mSubmitReportTransfer; //56
bool						SEHCatcher::mExiting = false;
bool						SEHCatcher::mShowUI = true;
bool						SEHCatcher::mAllowSubmit = true;

std::wstring					SEHCatcher::mCrashMessage = L"An unexpected error has occured!"; //64
std::string						SEHCatcher::mSubmitHost; //65
std::wstring					SEHCatcher::mSubmitMessage = L"Please help out by providing as much information as you can about this crash."; //66
std::wstring					SEHCatcher::mSubmitErrorMessage = L"Failed to connect to server."; //67
...
SEHCatcher::SEHCatcher()  //110-113
{ 	
	mPreviousFilter = EXCEPTION_CONTINUE_SEARCH;
	Init();
}

SEHCatcher::~SEHCatcher() //116-119
{
	if (mPreviousFilter)
		SetUnhandledExceptionFilter(mPreviousFilter); 
}

void SEHCatcher::Init() //122-127
{
	if (!mPreviousFilter && !mDisableCrashHandler)
	{
		mPreviousFilter = SetUnhandledExceptionFilter(UnhandledExceptionFilter);
	}
}

bool SEHCatcher::HandleCmdLineParam(const std::string& theParamName, const std::string& theParamValue) //130-143
{
	if (theParamName == "-minidump")
	{
		mAllowMinidumps = true;
		return true;
	}
	else if (theParamName == "-ultradump")
	{
		mAllowUltradumps = true;
		return true;
	}
...
void SEHCatcher::DisableCrashHandler() //146-150
{
	if (mPreviousFilter)
		SetUnhandledExceptionFilter(mPreviousFilter);
	mDisableCrashHandler = true;
}

long __stdcall SEHCatcher::UnhandledExceptionFilter(LPEXCEPTION_POINTERS lpExceptPtr) //153-163
{
	if (mApp != NULL)
		mApp->mAppDriver->SEHOccured();	
...
bool SEHCatcher::LoadImageHelp() //166-235
{
	mImageHelpLib = LoadLibraryA("DBGHELP.DLL");
	if (!mImageHelpLib)
	{
	...
	mKernelLib = LoadLibraryA("KERNEL32.DLL");
	mRtlCaptureContext = (RTLCAPTURECONTEXTPROC) GetProcAddress(mKernelLib, "RtlCaptureContext");
	mRtlCaptureStackBackTrace = (RTLCAPTURESTACKBACKTRACE) GetProcAddress(mKernelLib, "RtlCaptureStackBackTrace");
	...
	mSymGetLineFromAddr = (SYMGETLINEFROMADDR) GetProcAddress(mImageHelpLib, "SymGetLineFromAddr");
...
void SEHCatcher::UnloadImageHelp() //238-241
{
	if (mImageHelpLib != NULL)
		FreeLibrary(mImageHelpLib);
}

static bool StrToLongHex(const std::string& aString, DWORD* theValue) //244-265
{
	*theValue = 0;
...
void SEHCatcher::GetSymbolsFromMapFile(std::string &theDebugDump) //268-414
{
...
void SEHCatcher::DoHandleDebugEvent(LPEXCEPTION_POINTERS lpEP) //Correct? | 417-579
{
...
#ifdef _DEBUG //Only in Debug engine build.
	if (mAllowMinidumps)
	{
		if (mImageHelpLib)
		{
			...
			MINIDUMPWRITEPROC aMiniDumpWriteDumpProc = (MINIDUMPWRITEPROC)GetProcAddress(mImageHelpLib, "MiniDumpWriteDump");
			if (aMiniDumpWriteDumpProc)
			{
				wchar_t aDesktopFolder[1024];
				wchar_t aDumpFileName[1024];
				memset(aDesktopFolder, 0, sizeof aDesktopFolder);
				SHGetSpecialFolderPathW(0, aDesktopFolder, 0, 0);
				HANDLE aDumpFileHandle = NULL;
				for (int iSuffix = 0; iSuffix < 1000; iSuffix++)
				{
					swprintf_s(aDumpFileName, L"%s\\%s %s Crash %d.mdmp", aDesktopFolder, StringToWString(mApp->mProdName).c_str(), StringToWString(mApp->mProductVersion).c_str(), iSuffix);
					aDumpFileHandle = CreateFileW(aDumpFileName, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
					if (aDumpFileHandle != INVALID_HANDLE_VALUE)
						break;
				}
				if (aDumpFileHandle != INVALID_HANDLE_VALUE)
				{
					MINIDUMP_EXCEPTION_INFORMATION aDumpExInfo;
					aDumpExInfo.ThreadId = GetCurrentThreadId();
					aDumpExInfo.ExceptionPointers = lpEP;
					aDumpExInfo.ClientPointers = TRUE;
					MINIDUMP_TYPE aUltraDumpFlags = MiniDumpNormal; //Prob incorporates MINIDUMP_TYPE
					if (mAllowUltradumps)
						aUltraDumpFlags = MiniDumpWithFullMemory;
					//if (aMiniDumpWriteDumpProc == NULL)
					if (aMiniDumpWriteDumpProc(GetCurrentProcess(), GetCurrentProcessId(), aDumpFileHandle, (MINIDUMP_TYPE)(aUltraDumpFlags | MiniDumpWithDataSegs | MiniDumpWithHandleData), &aDumpExInfo, 0, 0) != NULL)
						CloseHandle(aDumpFileHandle);
					else
					{
						CloseHandle(aDumpFileHandle);
						DeleteFileW(aDumpFileName);
					}
				}
			}
		}
...
std::string SEHCatcher::IntelWalk(PCONTEXT theContext, int theSkipCount) //582-620
{
	std::string aDebugDump;
	char aBuffer[2048];
...
std::string SEHCatcher::ImageHelpWalk(PCONTEXT theContext, int theSkipCount) //623-738
{
	char aBuffer[2048];
	std::string aDebugDump;
	...
	for (;;)
	{
		...
		{
#ifdef _SEXYDECOMP_USE_LEGACY_CODE //Not sure why this was cut, Beef also cut it
		...
		if (theSkipCount > 0)
		{
			if (aLevelCount > 0) sf.AddrFrame.Offset -= 5;
			theSkipCount--;
			continue;
		}
		...
		if (mSymGetSymFromAddr(GetCurrentProcess(), sf.AddrPC.Offset, &symDisplacement, pSymbol))
		{
			...
			if (mSymGetLineFromAddr)
			{
				IMAGEHLP_LINE aLine;
				aLine.SizeOfStruct = 20;
				ulong aLineDisplacement;
				if (!mSymGetLineFromAddr(GetCurrentProcess(), sf.AddrPC.Offset, &aLineDisplacement, &aLine))
				{
					aLine.FileName = "(Unknown File Name)";
					aLine.LineNumber = 0;
					aLineDisplacement = 0;
				}
				sprintf(aBuffer, "%08X %08X %hs+%X [%s(%d)+0x%08X]\r\n", sf.AddrFrame.Offset, sf.AddrPC.Offset, aUDName, symDisplacement, aLine.FileName, aLine.LineNumber, aLineDisplacement);
			}
			else
...
bool SEHCatcher::GetLogicalAddress(void* addr, char* szModule, DWORD len, DWORD& section, DWORD& offset) //742-802
{
	offset = 0;
	section = 0;
	...
	if (!VirtualQuery(addr, &mbi, sizeof(mbi)))
	{
		if (mbi.State == MEM_FREE)
			strncpy(szModule, "Failed due to address in MEM_FREE pages", len);
		return false;
	}

    DWORD hMod = (DWORD)mbi.AllocationBase;
	if (!mbi.AllocationBase) (DWORD)mbi.BaseAddress;

    if (!GetModuleFileNameA((HMODULE) hMod, szModule, len))
		strncpy(szModule, "GetModuleFileName failed", len);
        return false;
	...
	strncpy(szModule, "VirtualQuery failed", len);
    return false; // Should never get here!
}

std::string SEHCatcher::GetFilename(const std::string& thePath) //805-814
{
...
static LRESULT CALLBACK SEHProgressWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) //820-877
{
	switch (uMsg)
	{		
	case WM_COMMAND:
...
static void CreateProgressWindow() //880-947
{
	WNDCLASSA wc;
	wc.style = 0;
	wc.cbClsExtra = 0;
...
LRESULT CALLBACK SEHCatcher::SEHWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) //950-980
{
	switch (uMsg)
	{
	case WM_COMMAND:
...
LRESULT CALLBACK SEHCatcher::SubmitInfoWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) //983-1013
{
	switch (uMsg)
	{		
	case WM_COMMAND:
...
void SEHCatcher::WriteToFile(const std::string& theErrorText) //1016-1019
{
	std::fstream aStream("crash.txt", std::ios::out);
	aStream << theErrorText.c_str() << std::endl;
}

void SEHCatcher::SubmitReportThread(void *theArg) //1022-1118
{
	std::string aSeperator = "---------------------------7d3e1f30eec";
	
	DefinesMap aSEHWebParams;

	mApp->mAppDriver->GetSEHWebParams(&aSEHWebParams);
	...
	if (mHasDemoFile) //Why is this still here
	{
		Buffer aBuffer;
...
void SEHCatcher::ShowSubmitInfoDialog() //1121-1231
{
	WNDCLASSA wc;
	wc.style = 0;
	wc.cbClsExtra = 0;
...
void SEHCatcher::ShowErrorDialog(const std::string& theErrorTitle, const std::string& theErrorText) //1234-1408
{
...
//#else
	bool doDebugButton = false; //Changed to always idk why
//#endif
...
#ifdef _DEBUG //Only visible on Debug as of Twist/ZR/PrePrime
	if (doDebugButton)
	{
...
std::string SEHCatcher::GetSysInfo() //1411-1462
{
	std::string aDebugDump;
	...
	if (mApp != NULL)
	{
		...
		if (aMod != NULL)
		{
			...
			aDebugDump += "DDraw Ver: " + mApp->mAppDriver->GetProductVersion(aPath) + "\r\n";
			FreeLibrary(aMod);
		}
		...
		{
			GetModuleFileNameA(aMod, aPath, 256);			
			aDebugDump += "DSound Ver: " + mApp->mAppDriver->GetProductVersion(aPath) + "\r\n";
			FreeLibrary(aMod);
		}
	}	
