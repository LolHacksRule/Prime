...
HTTPTransfer::HTTPTransfer() //14-20
{
...
HTTPTransfer::~HTTPTransfer() //23-29
{
	Abort();
...
std::string HTTPTransfer::GetAbsURL(const std::string& theBaseURL, const std::string& theRelURL) //32-65
{
	std::string aURL;
...
void HTTPTransfer::Fail(int theResult) //68-71
{	
	mResult = theResult;
	mExiting = true;
}

bool HTTPTransfer::SocketWait(bool checkRead, bool checkWrite) //74-123
{
	while (!mExiting)
	{
		fd_set aReadSet;
...
void HTTPTransfer::GetThreadProc() //126-335
{
#ifdef _WIN32 //Only appears on Windows.
	WSADATA aDat;
...
void HTTPTransfer::GetThreadProcStub(void *theArg) //338-340
{
	((HTTPTransfer*) theArg)->GetThreadProc();
}

void HTTPTransfer::PrepareTransfer(const std::string& theURL) //343-383
{
	Reset();	
...
void HTTPTransfer::StartTransfer() //386-391
{	
...
void HTTPTransfer::GetHelper(const std::string& theURL) //394-405
{
...
void HTTPTransfer::PostHelper(const std::string& theURL, const std::string& theParams) //408-421
{
	PrepareTransfer(theURL);
...
void HTTPTransfer::Get(const std::string& theURL) //424-428
{
	mSpecifiedBaseURL = "";
...
void HTTPTransfer::Post(const std::string& theURL, const std::string& theParams) //431-435
{
	mSpecifiedBaseURL = "";
	mSpecifiedRelURL = theURL;
...
void HTTPTransfer::Get(const std::string& theBaseURL, const std::string& theRelURL) //438-443
{
...
void HTTPTransfer::Post(const std::string& theBaseURL, const std::string& theRelURL, const std::string& theParams) //446-451
{
	mSpecifiedBaseURL = theBaseURL;
	mSpecifiedRelURL = theRelURL;
...
void HTTPTransfer::SendRequestString(const std::string& theHost, const std::string& theSendString) //454-460
{
...
void HTTPTransfer::Abort() //463-466
{	
	mAborted = true;
	mExiting = true;	
}

void HTTPTransfer::Reset() //469-485
{	
	if (mThreadRunning)
	{
		Abort();
...
static int aLastThreadId = 0; //You never even use this!

void HTTPTransfer::UpdateStatus() //490-491
{
}

void HTTPTransfer::WaitFor() //494-500
{
...
int HTTPTransfer::GetResultCode() //503-506
{
	UpdateStatus();
	return mResult;
}

std::string HTTPTransfer::GetContent() //509-515
{
