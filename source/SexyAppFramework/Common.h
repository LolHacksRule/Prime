...
#elif _SEXYDECOMP_USE_32BIT_WIDESTRING //Use PopCap's new 32 bit strings, HIGHLY recommended if using Resource Streams, UNUSED ON WINDOWS. MacOS surprisingly uses them | defined(_ANDROID) || defined(_APPLE) || defined(_XENON) || defined(_CELL) || defined(_ORBIS) || defined(_DURANGO) || defined(_OSX) && defined
typedef std::u32string		SexyString;
#define _S(x)				U ##x
#else //Windows probably


#define sexystrncmp			wcsncmp //Unused in Prime PC
#define sexystrcmp			wcscmp //Unused
#define sexystricmp			wcsicmp //Unused in Prime PC
#define sexysscanf			swscanf //Barely used
#define sexyatoi			_wtoi //Almost unused in Prime PC, used in EAMT
#define sexystrcpy			wcscpy //Unused in Prime PC

#define SexyStringToStringFast(x)	WStringToString(x, false)
...
#define WStringToSexyStringFast(x)	WStringToString(x, false)
...
typedef ulong uint32; //Add, confirmed according to matching hash
typedef __int64 int64;
typedef unsigned __int64 uint64; //Add

typedef std::map<std::string, std::string>		DefinesMap;
typedef std::map<std::wstring, std::wstring>	WStringWStringMap;
typedef std::map<std::wstring, std::string> WStringStringMap;
typedef SexyString::value_type					SexyChar;
typedef std::vector<std::string> StringVector; //move
...
void				GetProcessorInfoStrings(StringVector& outStrings);
...
std::string			WStringToString(const std::wstring &theString, bool *isValid);
...
bool				StringToInt(std::string theString, int* theIntVal);
bool				StringToDouble(std::string theString, double* theDoubleVal);
bool				StringToInt(std::wstring theString, int* theIntVal);
bool				StringToDouble(std::wstring theString, double* theDoubleVal);
...
SexyString			UCommaSeparate(uint theValue);
...
bool				FileExists(const std::string& theFileName, bool *isFolder);
bool				FileExists(const std::wstring& theFileName, bool *isFolder);
...
time_t				GetFileDate(const std::wstring& theFileName);
...
void				InitXprintf();
bool				WriteUTF8XMLFile(FILE* fp, const std::wstring& theData, bool includePreamble);
#endif
...
struct StringLessNoCase { bool operator()(const std::string &s1, const std::string &s2) const { return _stricmp(s1.c_str(),s2.c_str())<0; } }; //268

struct StringGreaterNoCase { bool operator()(const std::string &s1, const std::string &s2) const { return _stricmp(s1.c_str(),s2.c_str())>0; } };


void SexySleep(ulong milliseconds) //286-296
{
	Sleep(milliseconds); //on iOS, usleep
}

}

struct CharToCharFunc { static const char* Str(const char* theStr); static char Char(char theChar); };
struct CharToWCharFunc { static const std::wstring Str(const char* theStr); static wchar_t Char(char theChar); };
