...
PropertiesParser::PropertiesParser(SexyAppBase* theApp) //8-11
{
	mApp = theApp;
	mHasFailed = false;
	mXMLParser = 0;
...
void PropertiesParser::Fail(const SexyString& theErrorText) //15-25
{
	if (!mHasFailed)
	{
		mHasFailed = true;
		...
		if (!mXMLParser->GetFileName().empty()) mError += StrFormat(_S(" in File '%s'"), ToSexyString(mXMLParser->GetFileName()).c_str());
	}
}


PropertiesParser::~PropertiesParser() //29-30
{
}


bool PropertiesParser::ParseSingleElement(SexyString* aString) //34-57
{
	*aString = _S("");
...
bool PropertiesParser::ParseStringArray(StringVector* theStringVector) //60-96
{
...
bool PropertiesParser::ParseProperties() //100-205
{
	for (;;)
	{
		XMLElement aXMLElement;
...
bool PropertiesParser::DoParseProperties() //208-245
{
...
bool PropertiesParser::ParsePropertiesBuffer(const Buffer& theBuffer) //248-253
{
...
bool PropertiesParser::ParsePropertiesFile(const std::string& theFilename) //256-260
{
	mXMLParser = new XMLParser();
...
SexyString PropertiesParser::GetErrorText() //263-265
{
	return mError;
}
