...
static std::set<std::string> gStringPSet; //11
...
struct ModPointer
{
	...
	ModPointer(const char *theStrPtr, int theLineNum) : mStrPtr(theStrPtr), mLineNum(theLineNum) {} //28
};

typedef std::multimap<int, ModPointer> ModStorageMap; // stores counters
...
struct FileMod
{
	...
	FileMod(bool hasMods = false) { mHasMods = hasMods; } //38
};

typedef std::map<std::string, int> StringToIntMap;
typedef std::map<std::string, FileMod> FileModMap;

static StringToIntMap gStringToIntMap; //49
time_t gLastFileTime = 0;
const char *gSampleString = NULL; // for finding the others

static FileModMap& GetFileModMap() //54-57 (Matched)
{
	static FileModMap aMap;
	return aMap;
}

static const char* FindFileInStringTable(const std::string &theSearch, const char *theMem, DWORD theLength, const char *theStartPos) //60-76 (Matched)
{
...
static bool ParseModValString(std::string &theStr, int *theCounter = NULL, int *theLineNum = NULL) //79-107 (Matched)
{
...
static bool FindModValsInMemoryHelper(const char *theMem, DWORD theLength) //110-138
{
	...
	while (true)
	{
		...
		{
			/*if (aLineNum==4105) // This made it uncompilable on 64bit. It looks like it's still here on Prime.
				_asm nop;*/

			FileMod &aFileMod = aMap[aFileName];
			aFileMod.mMap.insert(ModStorageMap::value_type(aCounter, ModPointer(aPtr - 5, aLineNum)));
			foundOne = true;
		}
		aPtr++;
	}
...
static void FindModValsInMemory() //141-184 (Matched)
{
...
static ModStorage* CreateFileModsHelper(const char* theFileName) //187-200
{
...
static ModStorage* CreateFileMods(const char* theFileName) //204-215 (Matched)
{	
...
int Sexy::ModVal(int theAreaNum, const char* theFileName, int theInt) //218-227 (Matched)
{	
	if (*theFileName != 0)
...
double Sexy::ModVal(int theAreaNum, const char* theFileName, double theDouble) //230-239 (Matched)
{
	if (*theFileName != 0)
		CreateFileMods(theFileName);	
...
float Sexy::ModVal(int theAreaNum, const char* theFileName, float theFloat) //242-244 (Matched)
{
	return (float) ModVal(theAreaNum, theFileName, (double) theFloat);
}

const char*	Sexy::ModVal(int theAreaNum, const char* theFileName, const char *theStr) //247-256 (Matched)
{
	if (*theFileName != 0)
		CreateFileMods(theFileName);	
...
const char** Sexy::ModValP(int theAreaNum, const char* theFileName, const char* theStr) //259-273
{
	if (*theFileName != 0)
		CreateFileMods(theFileName);
	...
	if (!aModStorage->mStringPtr)
	{
		aModStorage->mString = theStr;
		std::pair<std::set<std::string>::iterator, bool> aPair = gStringPSet.insert(theStr);
		aModStorage->mStringPtr = aPair.first->c_str();
	}
	return &aModStorage->mStringPtr;
}

void Sexy::AddModValEnum(const std::string &theEnumName, int theVal) //276-278 (Matched)
{
	gStringToIntMap[theEnumName] = theVal;
}

static bool ModStringToInteger(const char* theString, int* theIntVal) //281-359 (Matched)
{
	*theIntVal = 0;
...
static bool ModStringToDouble(const char* theString, double* theDoubleVal) //354-416 (Matched)
{
	*theDoubleVal = 0.0;
...
static bool ModStringToString(const char* theString, std::string &theStrVal) //419-459 (Matched)
{
	if (theString[0]!='"')
		return false;
...
bool Sexy::ReparseModValues() //462-715 (Correct?)
{
	if (gLastFileTime == 0)
	{
		char anEXEName[256];
				...
				while (aString[aCharIdx] != 0)
				{
					...
					else if (aChar == '(')
					{
							...
							(!isalpha((unsigned char) aString[aCharIdx-2])))
						{
							...
							(aString[aCharIdx - 1] >= '1' && aString[aCharIdx - 1] <= '9' || aString[aCharIdx - 1] == 'S' || aString[aCharIdx - 1] == 'P') && //Not sure if it is correct. Original line (1.30): (aString[aCharIdx-1] >= '1') && (aString[aCharIdx-1] <= '9') &&
							(aString[aCharIdx-2] == 'M') &&
							(!isalpha((unsigned char) aString[aCharIdx-3])))
						{
							aCurId = aString[aCharIdx-1] - 'P';
