...
void Checkbox::Draw(Graphics* g)
{
	...
	if ((mCheckedRect.mWidth == 0) && (mCheckedImage != NULL) && (mUncheckedImage != NULL))
	{
		...
		else //XNA puts this under return prob decompiler stuff
...
void Checkbox::GamepadButtonDown(GamepadButton theButton, int player, GamepadButtonExta flags)
{
	if (theButton != GAMEPAD_BUTTON_A)
	{
		Widget::GamepadButtonDown(theButton, player, flags);
		return;
	}
	if (flags & GAMEPAD_BUTTON_REPEAT != 0)
		return;
	OnPressed();
	mChecked = !mChecked;
	mIsDown = true;
...
void Checkbox::GamepadButtonUp(GamepadButton theButton, int player, GamepadButtonExta flags)
{
	if (theButton == GAMEPAD_BUTTON_A)
		return;
	Widget::GamepadButtonUp(theButton, player, flags);
}
...
void Checkbox::MouseUp(int x, int y, int theBtnNum, int theClickCount)
{
