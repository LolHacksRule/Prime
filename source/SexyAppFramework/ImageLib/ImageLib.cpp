...
int	Image::GetWidth() //40-42
{
	return mWidth;
}

int	Image::GetHeight() //45-47
{
	return mHeight;
}

unsigned long* Image::GetBits() //50-52
{
	return mBits;
}
...
static void png_pak_read_data(png_structp png_ptr, png_bytep data, png_size_t length) //58-72
{
	png_size_t check;
...
void PngError(png_structp ptr, png_const_charp err) //75-77
{
	puts(err);
}

Image* GetPNGImage(const std::string& theFileName, int thePakLibSearchOrder) //Correct? | 80-174
{
	png_structp png_ptr;
	png_infop info_ptr;
	...
	if ((fp = p_fopen(theFileName.c_str(), "rb", thePakLibSearchOrder)) == NULL)
		return NULL;
...
Image* GetTGAImage(const std::string& theFileName, int thePakLibSearchOrder) //177-236 (Match?)
{
	PFILE* aTGAFile = p_fopen(theFileName.c_str(), "rb", thePakLibSearchOrder);
	if (aTGAFile == NULL)
		return NULL;
...
int ReadBlobBlock(PFILE* fp, char* data) //239-244 (Match)
{
...
bool ImageLib::GetGIFSize(PFILE* fp, int* width, int* height) //? | 247-289
{
	uchar magick[12];
	int status = p_fread(magick, sizeof(char), 6, fp);
	if (((strncmp((char*)magick, "GIF87", 5) != 0) &&
		(strncmp((char*)magick, "GIF89", 5) != 0)))
		return false;
	uint global_colors = 0;
	uchar* global_colormap = (unsigned char*)NULL;

	short pw;
	short ph;
	//Or p* = ES
	EndianShort(p_fread(&pw, sizeof(short), 1, fp));
	EndianShort(p_fread(&ph, sizeof(short), 1, fp));
	*width = pw;
	*height = ph;
	p_fseek(fp, 0, 0);
	return true;
...
bool ImageLib::LoadPNGToTextureAlpha(PFILE* fp, int expected_width, int expected_height, unsigned char* pBits, unsigned long Pitch) //Correct? | 292-376
{
	png_structp png_ptr;
	png_infop info_ptr;
...
bool ImageLib::LoadGIFToTextureAlpha(PFILE* fp, int expected_width, int expected_height, unsigned char* pBits, unsigned long Pitch) //TODO | 379-918
{
...
//	EndianShort(p_fread(&pw, sizeof(short), 1, fp));
//	EndianShort(p_fread(&ph, sizeof(short), 1, fp));
//	if (pw != expected_width || ph != expected_height)
//		return NULL;
//
...
//		bool interlaced;
//
//		EndianShort(p_fread(&pagex, sizeof(short), 1, fp));
//		EndianShort(p_fread(&pagey, sizeof(short), 1, fp));
//		EndianShort(p_fread(&width, sizeof(short), 1, fp));
//		EndianShort(p_fread(&height, sizeof(short), 1, fp));
//		EndianShort(p_fread(&flag, sizeof(char), 1, fp));
//
...
Image* GetGIFImage(const std::string& theFileName, int thePakLibSearchOrder) //Correct? | 921-1475
{
#define BitSet(byte,bit)  (((byte) & (bit)) == (bit))
#define LSBFirstOrder(x,y)  (((y) << 8) | (x))
...
METHODDEF(void)
my_error_exit(j_common_ptr cinfo) //1488-1499
{
...
void SetJpegTable(j_compress_ptr cinfo, int which_tbl, PNG_CONST uchar quantTables[0x40], int quality) //TODO | 1508-1557
{
	int aTabNum1;
	int aTabNum2;
	if (quality < 0)
	...
	if (quality > 100)
		quality = 100;
	if (cinfo->global_state != JTRC_SOF) //?
		ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
	if (which_tbl >= 4)
		ERREXIT1(cinfo, JERR_DQT_INDEX, which_tbl);
	JQUANT_TBL** qtblptr = (JQUANT_TBL**)cinfo[3].err + which_tbl;
	if (!qtblptr)
		*qtblptr = jpeg_alloc_quant_table((jpeg_common_struct*)cinfo);
	if (quality > 50)
	{
		aTabNum1 = quality / 10 + 1;
		aTabNum2 = quality / 10 + 2 <= 11 ? 11 : quality / 10 + 2;
	}
	else
	{
		aTabNum1 = quality / 10;
		aTabNum2 = quality / 10 + 1;
	}
	for (int i = 0; i < 64; ++i)
	{
		long temp = (10 * (quality % 10) * (unsigned char)quantTables[64 * aTabNum2 + i] + (100 - 10 * (quality % 10)) * (unsigned char)quantTables[64 * aTabNum1 + i]) / 100;
		if (temp <= 0)
			temp = 1;
		if (temp > 255)
			temp = 255; //LODWORD?
		(*qtblptr)->quantval[i] = temp;
	}
	(*qtblptr)->sent_table = false;
}

bool ImageLib::WriteJPEGImage(const std::string& theFileName, Image* theImage, int theQuality) //TODO CHANGED | 1560-1769
{
	PNG_CONST uchar LUM_QUANT[64][12] = {  }; //?
	PNG_CONST uchar CHROM_QUANT[64][12] = {  }; //?
	...
	SetJpegTable(&cinfo, 0, LUM_QUANT[0], theQuality);
	SetJpegTable(&cinfo, 1, CHROM_QUANT[0], theQuality);
...
bool ImageLib::WritePNGImage(const std::string& theFileName, Image* theImage, int theDPI) //Correct? | 1772-1858
{
	png_structp png_ptr;
	png_infop info_ptr;
   ...
   // set up your own error handlers in the png_create_write_struct() earlier.
	...
	if (theDPI)
	{
		const float inchesPerMeter = 39.370079;
		float pixelsPerMeter = (double)theDPI * inchesPerMeter;
		png_set_pHYs(png_ptr, info_ptr, (__int64)pixelsPerMeter, (__int64)pixelsPerMeter, 1);
	}
...
bool ImageLib::WriteTGAImage(const std::string& theFileName, Image* theImage) //1861-1907
{
...
bool ImageLib::WriteBMPImage(const std::string& theFileName, Image* theImage) //Why is this only true, was it deprecated (they prob commented the code and left it here) | 1910-1943
{
...
METHODDEF(void) init_source (j_decompress_ptr cinfo) //1961-1964
{
	pak_src_ptr src = (pak_src_ptr) cinfo->src;
	src->start_of_file = TRUE;
}

METHODDEF(boolean) fill_input_buffer (j_decompress_ptr cinfo) //1967-1989
{
	pak_src_ptr src = (pak_src_ptr) cinfo->src;
	size_t nbytes;
...
METHODDEF(void) skip_input_data (j_decompress_ptr cinfo, long num_bytes) //1992-2003
{
	pak_src_ptr src = (pak_src_ptr) cinfo->src;
...
METHODDEF(void) term_source (j_decompress_ptr cinfo) //2006-2008
{
	/* no work necessary here */
}

void jpeg_pak_src (j_decompress_ptr cinfo, PFILE* infile) //2011-2040
{
	pak_src_ptr src;
...
Image* GetJPEGImage(const std::string& theFileName, int thePakLibSearchOrder) //Correct? | 2044-2130
{
	PFILE *fp;
...
std::string gJ2KCodecKey = "Your registration here"; //2501

void ImageLib::InitJPEG2000() //? | 2504-2516
{
	uint aPathLen = MAX_PATH; //Is this redundant?
	wchar_t aPath[sizeof aPathLen];
	if (gJ2KCodec != NULL)
	{
		ZeroMemory(aPath, aPathLen);
		GetCurrentDirectoryW(MAX_PATH, aPath);
		if (aPath[aPathLen - 1] == '\\')
			wcscpy(&aPath[aPathLen], L"j2k-codec.dll");
		else
			wcscpy(&aPath[aPathLen], L"\\j2k-codec.dll");
		gJ2KCodec = LoadLibraryW(aPath);
	}
}

void ImageLib::CloseJPEG2000() //2519-2526
{
	if (gJ2KCodec != NULL)
	{
...
void ImageLib::SetJ2KCodecKey(const std::string& theKey) //2529-2531
{
	gJ2KCodecKey = theKey;
}

int __stdcall Pak_seek(void *data_source, int offset) //2534-2536
{
	return p_fseek((PFILE*) data_source, offset, SEEK_SET);
}

int __stdcall Pak_read(void *ptr, int size, void *data_source) //2539-2541
{
	return p_fread(ptr, 1, size, (PFILE*) data_source);
}

void __stdcall Pak_close(void *data_source) //2544-2545
{	
}

Image* GetJPEG2000Image(const std::string& theFileName, int thePakLibSearchOrder) //TODO CHANGED | 2548-2667
{
	if (gJ2KCodec != NULL)
	{
		PFILE* aFP = p_fopen(theFileName.c_str(), "rb", thePakLibSearchOrder);
		if (aFP == NULL)
			return NULL;
...
bool ImageLib::gJ2KLoadFuncs = true;
int ImageLib::gAlphaComposeColor = 0xFFFFFF;
bool ImageLib::gAutoLoadAlpha = true;
bool ImageLib::gIgnoreJPEG2000Alpha = true;

std::string ImageLib::gColorFileName = ""; //2675
std::string ImageLib::gAlphaFileName = ""; //2676

Image* GetImage_Phys(const std::string& theFilename, bool lookForAlphaImage, int thePakLibSearchOrder) //2679-2808
{
	if (!gAutoLoadAlpha)
	...
	{
		gColorFileName = aFilename + ".tga";
		anImage = GetTGAImage(gColorFileName, thePakLibSearchOrder);
	}

	if ((anImage == NULL) && ((stricmp(anExt.c_str(), ".jpg") == 0) || (anExt.length() == 0)))
	{
		gColorFileName = aFilename + ".jpg";
		anImage = GetJPEGImage(gColorFileName, thePakLibSearchOrder);
	}

	if ((anImage == NULL) && ((stricmp(anExt.c_str(), ".gif") == 0) || (anExt.length() == 0)))
	{
		gColorFileName = aFilename + ".gif";
		anImage = GetGIFImage(gColorFileName, thePakLibSearchOrder);
	}

	if (anImage == NULL && (stricmp(anExt.c_str(), ".j2k") == 0 || anExt.length() == 0))
	{
		gColorFileName = aFilename + ".j2k";
		anImage = GetJPEG2000Image(gColorFileName, thePakLibSearchOrder);
	}
	if (anImage == NULL && (stricmp(anExt.c_str(), ".jp2") == 0 || anExt.length() == 0))
	{
		gColorFileName = aFilename + ".jp2";
		anImage = GetJPEG2000Image(gColorFileName, thePakLibSearchOrder);
	}

	if ((anImage == NULL) && ((stricmp(anExt.c_str(), ".png") == 0) || (anExt.length() == 0)))
	{
		gColorFileName = aFilename + ".png";
		anImage = GetPNGImage(gColorFileName, thePakLibSearchOrder);
	}
	...
	{
		std::string anOldColorFileName = gColorFileName;
		// Check _ImageName
		anAlphaImage = GetImage(theFilename.substr(0, aLastSlashPos + 1) + "_" +
			theFilename.substr(aLastSlashPos + 1, theFilename.length() - aLastSlashPos - 1), false, thePakLibSearchOrder);

		// Check ImageName_
		if (anAlphaImage == NULL)
			anAlphaImage = GetImage(theFilename + "_", false, thePakLibSearchOrder);

		gAlphaFileName = gColorFileName;
		gColorFileName = anOldColorFileName;
	}
...
Image* ImageLib::GetImage(const std::string& theFilename, bool lookForAlphaImage, int thePakLibSearchOrder) //2811-2829
{
	if (thePakLibSearchOrder == -1)
		thePakLibSearchOrder = p_GetFileSearchOrder();

	if (thePakLibSearchOrder)
	{
		return GetImage_Phys(theFilename, lookForAlphaImage, thePakLibSearchOrder);
	}

	Image * anImage = GetImage_Phys(theFilename, lookForAlphaImage, PSEARCH_JUST_PAK);
	if (!anImage)
	{
		return GetImage_Phys(theFilename, lookForAlphaImage, PSEARCH_JUST_FILES);
	}
