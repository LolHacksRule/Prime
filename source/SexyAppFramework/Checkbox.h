...
public:
	...
	//New stuff, for now keeping GamepadButtonExta flags
#ifdef _SEXYDECOMP_USE_LATEST_CODE
	virtual void			MouseUp(int x, int y, int theClickCount) { Widget::MouseUp(x, y, theClickCount); } //The below two are part of Prime, only doing this for formatting
	virtual void			MouseUp(int x, int y, int theBtnNum, int theClickCount);
	virtual void			GamepadButtonDown(GamepadButton theButton, int player, GamepadButtonExta flags);
	virtual void			GamepadButtonUp(GamepadButton theButton, int player, GamepadButtonExta flags);
